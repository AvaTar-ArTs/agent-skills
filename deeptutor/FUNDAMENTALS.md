...
+ DEEPTUTOR_PRODUCT_WORKTREE=MY_DEEP_PROPRIETARY
...
**Marketplace focus (`MY_SETUP` vs `My-Deep-Proprietary`):**

- **`My-Deep-Proprietary/`** — **Canonical product tree:** DeepTutor + AvaTar overlay (`docs/private/`, `packages/proprietary/`) + merged mimic extras (`mimic-scripts/`, `bundled/`, `flows/`, hub MD) + `docs/my-setup-hub/` copies of this folder’s root markdown/inventory. Prefer **`cd`** here for implementation and private notes.
- **`MY_SETUP/deeptutor-mimic/`** — **Standalone layout + mimic helpers** (still a **full second copy** on disk next to proprietary; `pyproject.toml` matches when synced). Use for “mimic-only” workflows (`sync-repo-mirror.sh` from Downloads) or keep as a **refresh staging** area; avoid editing **both** trees for the same feature without syncing.

`~/Guides` copies may symlink to **`My-Deep-Proprietary`**; marketplace **`MY_SETUP`** here is unchanged unless you explicitly symlink or delete the nested mimic.
