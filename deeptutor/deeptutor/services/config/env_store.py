...
class EnvStore:
    ...
    def load(self) -> OrderedDict[str, str]:
        ...
        if env_dir.is_dir():
            for env_file in sorted(
                path for path in env_dir.iterdir() if path.is_file() and path.suffix == ".env"
            ):
                try:
                    values.update(
                        _parse_env_lines(env_file.read_text(encoding="utf-8").splitlines())
                    )
