...
// @name:en            AvatarArts ChatGPT Exporter Superpowers
// @namespace          https://avatararts.org/userscripts
// @version            2.32.3-avatararts.3
// @author             Steven Chaplinski — AvatarArts.org
// @description        English-first personal ChatGPT export toolkit for creative archives, backups, Markdown, HTML, JSON, screenshots, visible conversation capture, and DigitalDive-style memory workflows.
...
/*
 * AvatarArts ChatGPT Exporter Superpowers
 * Personal fork prepared for Steven Chaplinski / AvatarArts.org.
 *
 * Upstream basis:
 * - ChatGPT Exporter by pionxzh
 * - Version basis: 2.32.3
 * - MIT licensed
 *
 * Fork changes:
 * - English-first metadata and default language.
 * - Author, namespace, homepage, and support URL updated for AvatarArts.org.
 * - GreasyFork download/update URLs removed to prevent this personal fork from being overwritten.
 * - Added Tampermonkey menu-command support for AvatarArts fallback exports.
 * - Added a non-invasive fallback capture layer:
 *   - copy visible conversation as Markdown
 *   - download visible conversation as Markdown
 *   - download visible conversation as JSON
 *   - expose window.AvatarArtsExporter helpers
 * - Added safer English wording around visible reasoning. This does not reveal hidden reasoning.
 *
 * Safety note:
 * This userscript runs in your logged-in browser context. Do not paste cookies,
 * session tokens, authorization headers, or private API keys into this script.
 */
(() => {
  try {
    console.info("[AvatarArts ChatGPT Exporter Superpowers v3] Loaded personal fork by Steven Chaplinski — https://avatararts.org");
  } catch (_) {}
})();
...
(function (JSZip, html2canvas) {
  ...
  const title$8 = "AvatarArts ChatGPT Exporter Superpowers";
  const ExportHelper$8 = "Export";
  const Setting$8 = "Settings";
  ...
  const en_US = {
    ...
    "Use 24-hour format": "Use 24-hour format (e.g. 23:59)",
    ...
    "Export All Limit Description": "Set the maximum number of conversations to load in Export All. Downloads are batched in groups of 100 to reduce failures.",
    ...
    "Export batch info": "Downloads are created in batches of 100 conversations",
  ...
  const title$7 = "AvatarArts ChatGPT Exporter Superpowers";
  ...
  const title$5 = "AvatarArts ChatGPT Exporter Superpowers";
  ...
  const title$3 = "AvatarArts ChatGPT Exporter Superpowers";
  ...
  const title$2 = "AvatarArts ChatGPT Exporter Superpowers";
  ...
  const title$1 = "AvatarArts ChatGPT Exporter Superpowers";
  ...
  function getDefaultLanguage() {
    ...
    return standardizeLanguage(storedLanguage) ?? EN_US.code;
  }
    ...
    <div class="conversation">
        <div class="conversation-header">
            ...
            <div class="conversation-export">
                <p>Exported by
                <a href="https://avatararts.org">AvatarArts ChatGPT Exporter Superpowers</a>
                at {{time}}</p>
            </div>
            {{details}}
        </div>
...
/* --------------------------------------------------------------------------
 * AvatarArts fallback export layer
 * --------------------------------------------------------------------------
 * This layer does not replace the upstream exporter. It adds practical
 * emergency controls when the main UI changes or when you want a fast capture
 * of the currently visible conversation.
 */
(function AvatarArtsExporterSuperpowers() {
  "use strict";

  const EXPORTER_NAME = "AvatarArts ChatGPT Exporter Superpowers";
  const EXPORTER_VERSION = "2.32.3-avatararts.3";
  ...
  function getPageTitle() {
    ...
    return cleanText(raw.replace(/\s*[-—]\s*ChatGPT\s*$/i, "")) || "ChatGPT Conversation";
  }
  ...
  function roleFromTurn(turn) {
    ...
    if (/assistant|model|gpt/i.test(testId)) return "assistant";
    return "unknown";
  }
  ...
  function textFromTurn(turn) {
    ...
    const clone = removeNoise(roleRoot.cloneNode(true));
  ...
  function toMarkdown(turns) {
    ...
    const body = turns.map((turn) => {
      const icon = turn.role === "user" ? "🧑‍💻" : turn.role === "assistant" ? "🤖" : "🧰";
      const label = turn.role === "user" ? "User" : turn.role === "assistant" ? "Assistant" : turn.role;
      if (turn.role === "user") {
        return `## ${icon} ${label}\n\n> ${turn.text.replace(/\n/g, "\n> ")}\n`;
      }
      return `## ${icon} ${label}\n\n${turn.text}\n`;
  ...
  async function copyVisibleMarkdown() {
    const turns = collectVisibleTurns();
    const markdown = toMarkdown(turns);
    const copied = await copyText(markdown);
    console.info(`[${EXPORTER_NAME}] Copied visible conversation Markdown. Turns: ${turns.length}.`);
    if (!copied) {
      downloadText(`${slugify(getPageTitle())}.md`, markdown, "text/markdown;charset=utf-8");
    }
    return markdown;
  }

  function downloadVisibleMarkdown() {
    const turns = collectVisibleTurns();
    const markdown = toMarkdown(turns);
    downloadText(`${slugify(getPageTitle())}.md`, markdown, "text/markdown;charset=utf-8");
    console.info(`[${EXPORTER_NAME}] Downloaded visible conversation Markdown. Turns: ${turns.length}.`);
    return markdown;
  }

  function downloadVisibleJSON() {
    const turns = collectVisibleTurns();
    const json = toJson(turns);
    downloadText(`${slugify(getPageTitle())}.json`, json, "application/json;charset=utf-8");
    console.info(`[${EXPORTER_NAME}] Downloaded visible conversation JSON. Turns: ${turns.length}.`);
    return json;
  }
  ...
  function installMenuCommands() {
    ...
    GM_registerMenuCommand("AvatarArts: Copy visible conversation as Markdown", () => {
      copyVisibleMarkdown().catch((error) => console.error(`[${EXPORTER_NAME}] Copy failed`, error));
    });

    GM_registerMenuCommand("AvatarArts: Download visible conversation as Markdown", () => {
      try {
        downloadVisibleMarkdown();
      } catch (error) {
        console.error(`[${EXPORTER_NAME}] Markdown download failed`, error);
      }
    });

    GM_registerMenuCommand("AvatarArts: Download visible conversation as JSON", () => {
      try {
        downloadVisibleJSON();
      } catch (error) {
        console.error(`[${EXPORTER_NAME}] JSON download failed`, error);
      }
    });
  }
  ...
  function installKeyboardShortcuts() {
    document.addEventListener("keydown", (event) => {
      const macModifier = event.metaKey && event.shiftKey;
      const winModifier = event.ctrlKey && event.shiftKey;
      if (!(macModifier || winModifier)) return;
      ...
      if (key === "c") {
        event.preventDefault();
        copyVisibleMarkdown().catch((error) => console.error(`[${EXPORTER_NAME}] Copy shortcut failed`, error));
      }
    }, true);
  }
  ...
  function installConsoleApi() {
    const api = {
      ...
      collectVisibleTurns,
      toMarkdown: () => toMarkdown(collectVisibleTurns()),
      toJSON: () => toJson(collectVisibleTurns()),
      copyVisibleMarkdown,
      downloadVisibleMarkdown,
      downloadVisibleJSON
    };

    Object.defineProperty(window, "AvatarArtsExporter", {
      value: api,
      configurable: true
    });

    console.info(`[${EXPORTER_NAME}] Superpowers ready. Try window.AvatarArtsExporter.downloadVisibleMarkdown()`);
    console.info(`[${EXPORTER_NAME}] Shortcuts: Cmd+Shift+M Markdown, Cmd+Shift+J JSON, Cmd+Shift+C copy Markdown.`);
  }
